...
class UserServiceImplTest {
    ...
    void getAllUsers() {
        ...
        users.add(user1);
        users.add(user2);
        Page<User> mockResult = new PageImpl<>(users);
        Mockito.when(mockUserRepository.findAll(Mockito.any(PageRequest.class))).thenReturn(mockResult);

        List<UserDto> allUsers = userService.getAllUsers(0, 10);
